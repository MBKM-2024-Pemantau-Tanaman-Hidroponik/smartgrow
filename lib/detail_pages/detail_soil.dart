...
class DetailSoil extends StatefulWidget {
  const DetailSoil({super.key});

  @override
  State<DetailSoil> createState() => _DetailSoilState();
}

class _DetailSoilState extends State<DetailSoil> {
  ...
  List<Map<String, dynamic>> _parseSensorData(Map<dynamic, dynamic> data) {
    return data.entries
        .where((entry) => entry.value.containsKey('soil_moisture'))
        .map((entry) {
      return {
        'id': entry.key,
        'soil_moisture': entry.value['soil_moisture'],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
          backgroundColor: const Color.fromARGB(255, 246, 245, 245),
          title: const Text('Soil Moisture Detail')),
