...
class DetailHumidity extends StatefulWidget {
  const DetailHumidity({super.key});

  @override
  State<DetailHumidity> createState() => _DetailHumidityState();
}

class _DetailHumidityState extends State<DetailHumidity> {
  ...
  List<Map<String, dynamic>> _parseSensorData(Map<dynamic, dynamic> data) {
    return data.entries
        .where((entry) => entry.value.containsKey('humidity'))
        .map((entry) {
      return {
        'id': entry.key,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color.fromARGB(255, 246, 245, 245),
        title: const Text('Humidity Detail'),
      ),
