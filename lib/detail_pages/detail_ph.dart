...
class DetailPH extends StatefulWidget {
  ...
  @override
  State<DetailPH> createState() => _DetailPHState();
}

class _DetailPHState extends State<DetailPH> {
  ...
  List<Map<String, dynamic>> _parseSensorData(Map<dynamic, dynamic> data) {
    return data.entries
        .where((entry) => entry.value.containsKey('pH')) // Filter only pH data
        .map((entry) {
      return {
        'id': entry.key,
        'pH': entry.value['pH'], // Extract pH value
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
          backgroundColor: const Color.fromARGB(255, 246, 245, 245),
          title: const Text('pH Water Detail')),
